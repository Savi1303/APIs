...
function getSubList() {
  const satSub = new  URLSearchParams(window.location.search);
  let subId = satSub.get('id');

  const getScrollItem = document.querySelector("subItems");
  getScrollItem.style.display = "block";
...
const url = `https://pluralcodesandbox.com/yorubalearning/api/admin/category_details/${subId}`;
    ...
    .then(result => {
      ...
      result?.map((item) => {
        ...
        `
        const info = document.querySelector(".row");
        info.innerHTML = data;
    })
    })
    .catch((error) => console.log("error", error));
